...
export default function RecommendationBlocks(props) {
  ...
  const columns = () => {
    let columns = [{
      ...
      render: (data) => {
        return <Link
          to={`/recommendation/bundle/${data.bundle_id}/${data.bundle_uid}`}>{data.bundle_name}</Link>
      }
    }, {
      field: 'weight',
      title: 'Weight'
  ...
  const onUpdateHandler = (e, data) => {
    e.preventDefault();
    history.push(`/recommendation/block/${data.id}/${data.uid}`);
  };
  ...
  const onDeleteDialogConfirmHandler = () => {
    ...
    dispatch(recommendationOperations.deleteRecommendationBlock(currentItem.id, currentItem.uid)).then(() => {
  ...
  useEffect(() => {
    dispatch(recommendationOperations.fetchRecommendationBlocks());
  }, [dispatch, runtime]);
